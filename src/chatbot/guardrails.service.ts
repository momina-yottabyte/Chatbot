...
export class GuardrailsService {
  ...
  /**
   * Clearly harmful / off-domain requests.
   * Only block things that have zero place in an HR assistant.
   */
  private readonly BLOCKED_PATTERNS: RegExp[] = [
    // Hacking / exploitation
    /\b(hack|exploit|vulnerability|sql\s*injection|xss|ddos|malware|phishing)\b/i,
    // Explicit harmful content
    /\b(bomb|weapon|drug|narcotic|suicide|self.?harm)\b/i,
    // Trying to extract system internals
    /\b(system\s+prompt|api\s+key|secret\s+key|access\s+token|dump\s+(the\s+)?(database|db))\b/i,
  ];
  ...
  private checkLength(message: string): GuardrailResult {
    ...
    if (message.length > this.MAX_INPUT_LENGTH) {
      return { allowed: false, reason: `Message exceeds ${this.MAX_INPUT_LENGTH} character limit` };
    }
    return { allowed: true };
  }

  private checkEncoding(message: string): GuardrailResult {
    const nonPrintable = (message.match(/[\x00-\x08\x0E-\x1F\x7F]/g) ?? []).length;
    if (nonPrintable / message.length > 0.6) {
      return { allowed: false, reason: 'Message contains excessive non-printable characters' };
    }
    return { allowed: true };
  ...
  private checkInjection(message: string): GuardrailResult {
    for (const pattern of this.INJECTION_PATTERNS) {
      if (pattern.test(message)) {
        return { allowed: false, reason: 'Potential prompt injection attempt detected' };
      }
    }
    return { allowed: true };
  }

  private checkBlocked(message: string): GuardrailResult {
    for (const pattern of this.BLOCKED_PATTERNS) {
      if (pattern.test(message)) {
        return { allowed: false, reason: 'This type of request cannot be handled by the HRMS assistant' };
      }
    }
    return { allowed: true };
  }
