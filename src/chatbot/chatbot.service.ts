  ...
  async handleMessage(message: string): Promise<string> {
    ...
    if (!inputCheck.allowed) {
      throw new BadRequestException(
        inputCheck.reason ?? 'Message did not pass validation',
      );
    }
    ...
    const intentResponse = this.matchIntent(message);
    if (intentResponse) {
      // Intent responses are pre-written, still run output filter for safety
      return this.guardrails.filterOutput(intentResponse);
    }
    ...
    const raw = response.choices[0].message.content ?? 'No response';
